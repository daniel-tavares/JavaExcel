...
import br.gov.pa.sefa.nfae.core.util.ConverterDados;
import jxl.Sheet;
import jxl.Workbook;
	...
	public static void main(String[] args) throws Exception {
		 ...
		 for (int l = 1; l <= 582; l++) {
				...
				if(!sheet.getCell(12,l).getContents().equals(""))
				  sb.append("Exceção Operação Interestadual: "+sheet.getCell(12,l).getContents());
				if(!sheet.getCell(13,l).getContents().equals(""))
				  sb.append("Legislação Operação Interestadual: "+sheet.getCell(13,l).getContents());
				if(!sheet.getCell(14,l).getContents().equals(""))
				   sb.append("Notas Operação Interestadual: "+sheet.getCell(14,l).getContents());
				if(!sheet.getCell(15,l).getContents().equals(""))
				   sb.append("Exceção Operação Interna: "+sheet.getCell(15,l).getContents());
				if(!sheet.getCell(16,l).getContents().equals(""))
				   sb.append("Legislação Operação Interna: "+sheet.getCell(16,l).getContents());
				if(!sheet.getCell(17,l).getContents().equals(""))
				   sb.append("Notas Operação Interna: "+sheet.getCell(17,l).getContents());
		 ...
		 for (Produto produto : listaProduto) {
    		 ...
    		 for (NCM ncm : produto.getListaNcm()) {
